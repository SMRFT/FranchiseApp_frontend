...
const StatusBadge = styled.span`
  ...
  font-weight: 700;
  background: ${({ status }) =>
    status === "Collected" ? "#ecfdf5" : "#fff7ed"};
  color: ${({ status }) =>
    status === "Collected" ? "#059669" : "#d97706"};
  border: 1px solid ${({ status }) =>
    status === "Collected" ? "#a7f3d0" : "#fed7aa"};
`
...
const SampleCollection = () => {
  ...
  // testSelections: { [testKey]: boolean } - whether checkbox is ticked
  const [testSelections, setTestSelections] = useState({})
  // testStatuses: { [testKey]: "Pending" | "Collected" }
  ...
  // Open modal: load patient data, fetch existing sample,
  // show only Pending tests (skip already-Collected ones)
  const openModal = async (patient) => {
    ...
    originalTests.forEach((test, index) => {
      ...
      const container = test.collection_container || "Plain/Gel"
      ...
      if (currentSampleData && currentSampleData.testdetails) {
        ...
        if (collectedTest) {
          existingStatus = collectedTest.samplestatus || "Pending"
        }
      }

      if (existingStatus === "Collected") {
        // Already collected — skip this test (won't render)
        initialSelections[testKey] = null   // null = already collected, hidden
        initialStatuses[testKey] = "Collected"
      } else if (existingStatus === "Pending") {
        // Previously saved as pending — show it, pre-checked, default Collected
        initialSelections[testKey] = true
        initialStatuses[testKey] = "Collected"
      } else {
        // New test — show it, pre-checked, default Collected
        initialSelections[testKey] = true
        initialStatuses[testKey] = "Collected"
      }
    })
  ...
  // Select all visible (non-collected) tests
  const handleSelectAll = (selectAll) => {
    ...
    tests.forEach((_, index) => {
      ...
      // Only toggle tests that are not already collected (null = hidden)
  ...
  const handleStatusChange = (testKey, status) => {
    ...
    // If set to collected, ensure it's selected
    if (status === "Collected") {
  ...
  const saveTestData = async () => {
    ...
    try {
      ...
      // Only send tests that are visible (not already collected) and are checked
      const formattedTestDetails = originalTests
        .map((test, index) => {
          const testKey = `${selectedPatient.patient_id}_${index}`
          const isVisible = testSelections[testKey] !== null  // null = already collected hidden
          const isSelected = testSelections[testKey] === true
          const currentStatus = testStatuses[testKey] || "Pending"

          // Skip already-collected (hidden) tests
  ...
  // Visible tests = those not already collected (null selection means hidden)
  const visibleTests = selectedPatient
    ? parseTestDetails(selectedPatient.testdetails).filter((_, index) => {
        const testKey = `${selectedPatient.patient_id}_${index}`
        return testSelections[testKey] !== null
      })
    : []

  const hasSelectedTests = Object.entries(testSelections).some(
    ([, val]) => val === true
  )

  // For select-all checkbox state (only considers visible tests)
  const allVisibleSelected =
    visibleTests.length > 0 &&
    selectedPatient &&
    parseTestDetails(selectedPatient.testdetails).every((_, index) => {
      const testKey = `${selectedPatient.patient_id}_${index}`
                ...
                {filteredPatients.map((patient) => (
                  <TableRow key={patient._id || patient.patient_id}>
                    ...
                    <TableCell data-label="Action">
                      <ViewButton onClick={() => openModal(patient)}>
                        View / Collect
                      </ViewButton>
                    </TableCell>
                  </TableRow>
                        ...
                        <TestTable>
                          ...
                          <tbody>
                            {parseTestDetails(selectedPatient.testdetails).map((test, index) => {
                              const testKey = `${selectedPatient.patient_id}_${index}`
                              // Skip already-collected tests (null selection)
                              ...
                              return (
                                <TestTableRow key={index}>
                                  ...
                                  <TestTableCell>
                                    <TestContainerBadge>
                                      {test.collection_container || "Plain/Gel"}
                                    </TestContainerBadge>
                                  </TestTableCell>
                                  <TestTableCell>
                                  ...
                                  <TestTableCell>
                                    <Select
                                      ...
                                      onChange={(e) =>
                                        handleStatusChange(testKey, e.target.value)
                                      }
                                    >
