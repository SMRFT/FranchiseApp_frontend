...
const PatientRegistrationAndBilling = () => {
  ...
  const handleRegistration = async (e) => {
    ...
    try {
      ...
      showToast("danger", "Failed", err.response?.data?.message || "Registration failed")
    }
  }
  ...
  const handleBillingConfirmation = async () => {
    try {
      ...
      showToast("danger", "Failed", "Billing confirmation failed")
    }
  }
  ...
  const handleSearch = async () => {
    ...
    } catch (e) { showToast("danger", "Error", "Search failed") }
  }

  const isReadOnly = registrationStatus !== "pending"
