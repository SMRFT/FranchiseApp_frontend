...
const RefBy = ({ show, setShow, onRefByAdded }) => {
  ...
  const handleRefBySave = async (e) => {
    e.preventDefault();
    try {
      const dataToSend = { ...formData, franchise_id: storedFranchiseId };
      await axios.post(`${franchiseurl}refby/`, dataToSend);
    ...
    <StyledModal show={show} onHide={handleClose} centered>
      <ModalHeader>
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          ...
          {storedFranchiseId && <span style={{ fontSize: '0.8rem', color: '#718096' }}>Franchise ID: {storedFranchiseId}</span>}
        </div>
