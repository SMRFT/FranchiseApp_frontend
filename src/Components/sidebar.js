...
import { Print, Settings, LocalShipping, FolderOpen, Description, HomeWork } from '@mui/icons-material';
...
// Mobile Top Bar for Phone screens
const MobileTopBar = styled.div`
  ...
  right: 0;
  height: 56px;
  background: linear-gradient(135deg, #4B9EB0 0%, #3a8596 100%);
  color: white;
  z-index: 995;
  display: flex;
  align-items: center;
...
const SidebarLayout = ({ children }) => {
  ...
  const navGroups = [
    ...
    },
    {
      title: 'Operations',
      items: [
        { name: 'HomeCollection', icon: HomeWork, label: 'Home Collection' },
        ...
        { name: 'MaterialRequisition', icon: Description, label: 'Material Requisition' },
      ]
    },
    {
  ...
  return (
    <LayoutContainer>
      ...
      <MobileTopBar>
        <MobileMenuButton onClick={toggleMobileSidebar} title="Open Menu">
          <Menu size={20} />
        </MobileMenuButton>
        <MobileBrand>Diagnostics Franchise</MobileBrand>
        <MobileBadge>{franchiseId}</MobileBadge>
      </MobileTopBar>
